                    ...
                    if chord_audio is not None:
                        ...
                        if generate_inversions and inversions:
                            for inv_num, inv_semitones in inversions:
                                ...
                                if selected_inversions is not None:
                                    if isinstance(selected_inversions, dict):
                                        ...
                                        if (
                                            ...
                                            continue
                                    elif inv_num not in selected_inversions:
                                        continue
...
def interactive_mode():
    ...
    if options_dict["chords"]:
        ...
        # Show all chord types by default, let user deselect
        selected_chord_types = {}

        # Get all chord types for all qualities
        ...
        if options_dict["chords"]:
            inversion_mode = questionary.select(
                ...
                choices=[
                    ...
                    "Select specific inversions per chord",
                    "No inversions",
                ],
                style=custom_style,
            ).ask()
            ...
            elif inversion_mode == "Select specific inversions per chord":
                generate_inversions = True
                selected_inversions = {}

                # For each selected chord type, show its possible inversions
                for quality, chord_types in selected_chord_types.items():
                    for chord_type in chord_types:
                        # Find the chord definition to get number of notes
                        chord_def = next(
                            (
                                chord
                                for chord in CHORD_DEFINITIONS
                                if chord[0] == chord_type
                            ),
                            None,
                        )
                        if (
                            chord_def and chord_def[3] >= 3
                        ):  # Only if chord has 3 or more notes
                            max_inversions = chord_def[3] - 1
                            ...
                            inversion_choices = [
                                get_ordinal(i) for i in range(1, max_inversions + 1)
                            ]

                            selected = questionary.checkbox(
                                f"Select inversions for {chord_type} ({quality}):",
    ...
    if options_dict["chords"]:
        ...
        if generate_inversions and selected_inversions is not None:
            if isinstance(selected_inversions, dict):
                print("Selected inversions per chord:")
                for (quality, chord_type), inversions in selected_inversions.items():
                    if inversions:
                        print(
                            f"  {chord_type} ({quality}): {', '.join(str(i) for i in inversions)}"
                        )
            elif selected_inversions:
                print(
